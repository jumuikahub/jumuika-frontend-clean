  ...
  return (
    <html lang="en">
      <body className="min-h-screen bg-white text-slate-900 antialiased">
        ...
        <header className="sticky top-0 z-30 w-full border-b bg-white/80 backdrop-blur">
          <div className="mx-auto flex h-14 max-w-screen-xl items-center justify-between px-4 sm:h-16">
            <Link href="/" className="flex items-center gap-2" aria-label="Jumuika Hub KE">
              {/* NOTE: removed onError (server components can't pass function props) */}
              <Image
                src="/logo-mark.svg"
                alt="Jumuika Hub KE"
                width={28}
            ...
            <nav className="hidden items-center gap-6 text-sm sm:flex">
              ...
              <Link href="/institutions" className="transition hover:text-emerald-700">
                Institutions
              </Link>
              <Link href="/how-it-works" className="transition hover:text-emerald-700">
                How It Works
              </Link>
              <Link href="/internlink" className="transition hover:text-emerald-700">
                InternLink
              </Link>
            </nav>
          </div>
        ...
        {/* Main */}
        <main className="mx-auto w-full max-w-screen-xl px-4">{children}</main>
